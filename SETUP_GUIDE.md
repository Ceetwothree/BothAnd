...
6. **`rls_tests.sql` codifies the direct-test method above into a runnable script** — creates two throwaway private orgs, simulates cross-org reads/writes as a real non-member via the same `SET LOCAL ROLE authenticated` method, and rolls everything back at the end either way. Run it (SQL editor, or `psql "$DATABASE_URL" -f rls_tests.sql`) after any change touching RLS policies, tenancy-relevant tables, or `SECURITY DEFINER` functions -- it's not wired into CI yet, so it only protects against a regression if it's actually run by hand.
